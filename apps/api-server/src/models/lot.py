...
class LotItem(Base, TimestampMixin):
    ...
    safety_guidance_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), nullable=True)
...
class LotImage(Base):
    ...
    is_proof_of_collection: Mapped[bool] = mapped_column(Boolean, default=false if False else False, nullable=False)
